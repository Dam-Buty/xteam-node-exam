 ...
 */
 const readTagsFile = cb => {
   fs.readFile(TAGS_FILE, 'utf8', (err, data) => {
     if (err) {
       if (err.code === "ENOENT") {
         console.error("XXX - File " + TAGS_FILE + " could not be found")
         cb(err)
       } else {
         console.error("XXX - There was an error reading the " + TAGS_FILE + " file :")
         console.error(err)
         cb(err)
       }
     }
...
const processArguments = cb => {
  ...
  readStdin(stdin => {
    if (stdin) {
      console.log("====> Got tags from stdin :")
      sanitizedCb(stdin)
    } else {
      if (process.argv[2]) {
        console.log("====> Got tags from argument :")
        sanitizedCb(process.argv[2])
      } else {
        readTagsFile((err, tagsFile) => {
          if (!err) {
            console.log("====> Got tags from tags file :")
